...
class AIOTaskConsumer:
    ...
    def __init__(self):
        ...
        self.processed_collection = self.db.processed_messages
    ...
    async def start_consumers(self):
        """启动所有 Kafka 消费者"""
        self.running = True
        await self.processed_collection.create_index("message_id", unique=True)
        await self.processed_collection.create_index("processed_at", expireAfterSeconds=604800)
        for topic, processor in self.processors.items():
            wrapped_processor = self.wrap_idempotent_processor(processor, topic, self.processed_collection)
            task = asyncio.create_task(
                kafka_manager.consume_messages(topic, f"{topic}_group", wrapped_processor)
            )
