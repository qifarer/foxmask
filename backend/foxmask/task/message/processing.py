...
from foxmask.file.services.management import file_service
from foxmask.file.services.upload import upload_service

from foxmask.knowledge.services.knowledge_item import knowledge_item_service
from foxmask.knowledge.services.knowledge_graph import knowledge_graph_service
from foxmask.knowledge.services.vector_search import vector_search_service

class ProcessingService:
    def __init__(self):
        self.file_repository = get_repository_manager().file_repository
        self.task_file_repository = get_repository_manager().upload_task_file_repository
        pass
    ...
    async def process_knowledge(self, message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            match event_type_enum:
                case MessageEventTypeEnum.CREATE_FILE_FROM_UPLOAD:
                    return await self._process_create_file_from_upload(message)
                case MessageEventTypeEnum.CREATE_ITEM_FROM_FILE:
                    return await self._process_create_item_from_file(message)
                case MessageEventTypeEnum.CREATE_ITEM_FROM_CHAT:
                    return await self._process_create_item_from_chat(message)
    ...
    async def _process_create_file_from_upload(self, message: Dict[str, Any]):
        logger.info(f"_process_create_file_from_upload:{message}")
        """Process a file creation request from a upload task file"""
        try:
            ...
            file = await file_service.create_file_from_upload(tenant_id=tenant_id,file_id=file_id)

            # 触发进一步处理
            processing_message = {
                "item_id": file.uid,
                "tenant_id": file.tenant_id,
                "type": MessageEventTypeEnum.CREATE_ITEM_FROM_FILE.value,
            }
    ...
    async def _process_create_item_from_file(self, message: Dict[str, Any]):
    ...
    async def _process_create_item_from_chat(self, message: Dict[str, Any]):
