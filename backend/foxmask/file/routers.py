...
from foxmask.file.models import FileMetadata  # 添加这行导入
...
async def upload_chunk(
    ...
    service = FileService()
    await service.upload_chunk(file_id, chunk_index, chunk_data, current_user.id, current_user)
...
async def complete_upload(
    ...
    service = FileService()
    file_meta = await service.complete_upload(file_id, current_user.id, current_user)
    return file_meta
...
async def get_download_url(
    ...
    service = FileService()
    url = await service.get_presigned_url(file_id, current_user.id, current_user, expires)
    return {"url": url}
...
async def delete_file(
    ...
    service = FileService()
    await service.delete_file(file_id, current_user.id, current_user)
    return None