...
from foxmask.file.enums import FileTypeEnum, UploadProcStatusEnum, UploadTaskTypeEnum,UploadSourceTypeEnum,UploadStrategyEnum
...
class UploadTask(MasterBaseModel):
    ...
    # 基础信息
    proc_status: UploadProcStatusEnum = Field(default=UploadProcStatusEnum.PENDING, description="任务状态")
    # 上传源信息
    source_type: UploadSourceTypeEnum = Field(..., description="上传源类型")
    source_paths: List[str] = Field(..., description="源路径列表")
    
    # 上传配置
    upload_strategy: UploadStrategyEnum = Field(default=UploadStrategyEnum.SEQUENTIAL, description="上传策略")
    ...
    # 增强的进度统计
    discovered_files: int = Field(default=0, ge=0, description="已发现文件数")
    processing_files: int = Field(default=0, ge=0, description="处理中文件数")
    ...
    total_size: int = Field(default=0, ge=0, description="总文件大小")
    uploaded_size: int = Field(default=0, ge=0, description="已上传大小")
     
    # 时间戳
    discovery_started_at: Optional[datetime] = Field(None, description="文件发现开始时间")
    discovery_completed_at: Optional[datetime] = Field(None, description="文件发现完成时间")
...
class UploadTaskFile(SlaveBaseModel):
    """优化后的上传任务文件模型"""
    proc_status: UploadProcStatusEnum = Field(default=UploadProcStatusEnum.PENDING, description="文件状态")
    ...
    minio_object_name: Optional[str] = Field(None, description="MinIO对象名称")
    ...
    uploaded_chunks: int = Field(default=0, ge=0, description="已上传分块数")
    current_chunk: int = Field(default=0, ge=0, description="当前分块索引")
    
    # 进度信息
    progress: float = Field(default=0.0, ge=0.0, le=100.0, description="上传进度")
    upload_speed: Optional[float] = Field(None, description="上传速度")
    estimated_time_remaining: Optional[float] = Field(None, description="预计剩余时间")
    
    # 自动提取的元数据
    extracted_metadata: Dict[str, Any] = Field(default_factory=dict, description="自动提取的元数据")
    # 时间戳
    upload_started_at: Optional[datetime] = Field(None, description="上传开始时间")
    upload_completed_at: Optional[datetime] = Field(None, description="上传完成时间")
...
class UploadTaskFileChunk(SlaveBaseModel):
    ...
    # 字节范围
    start_byte: int = Field(..., ge=0, description="起始字节")
    end_byte: int = Field(..., ge=0, description="结束字节")
