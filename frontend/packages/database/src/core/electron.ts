...
declare global {
  ...
  var __FOXMASK__: LobeGlobal;
}

if (!globalThis.__FOXMASK__) {
  globalThis.__FOXMASK__ = {};
}
...
const acquireLock = async (dbPath: string): Promise<boolean> => {
  try {
    ...
    if (!fs.existsSync(lockPath)) {
      ...
      // 保存锁信息到全局对象
      if (!globalThis.__FOXMASK__.pgDBLock) {
        globalThis.__FOXMASK__.pgDBLock = {
          acquired: true,
          lockPath,
        };
      }
 ...
 */
const releaseLock = () => {
  if (globalThis.__FOXMASK__.pgDBLock?.acquired && globalThis.__FOXMASK__.pgDBLock.lockPath) {
    try {
      fs.unlinkSync(globalThis.__FOXMASK__.pgDBLock.lockPath);
      globalThis.__FOXMASK__.pgDBLock.acquired = false;
      console.log(`✅ Released database lock: ${globalThis.__FOXMASK__.pgDBLock.lockPath}`);
    } catch (error) {
...
const checkAndCleanupExistingInstance = async () => {
  if (globalThis.__FOXMASK__.pgDB) {
    try {
      // 尝试关闭现有的 PGlite 实例 (如果客户端有 close 方法)
      // @ts-expect-error
      const client = globalThis.__FOXMASK__.pgDB?.dialect?.client;
      ...
      // 重置全局引用
      globalThis.__FOXMASK__.pgDB = undefined;
    } catch (error) {
...
export const getPgliteInstance = async (): Promise<LobeChatDatabase> => {
  try {
    console.log(
      ...
      JSON.stringify({
        hasExistingDB: !!globalThis.__FOXMASK__.pgDB,
        hasPromise: !!globalThis.__FOXMASK__.pgDBInitPromise,
        isInitializing,
      }),
    );

    // 已经初始化完成，直接返回实例
    if (globalThis.__FOXMASK__.pgDB) return globalThis.__FOXMASK__.pgDB;

    // 有初始化进行中的Promise，等待它完成
    if (globalThis.__FOXMASK__.pgDBInitPromise) {
      console.log('Waiting for existing initialization promise to complete');
      return globalThis.__FOXMASK__.pgDBInitPromise;
    }
      ...
      return new Promise((resolve, reject) => {
        const checkInterval = setInterval(() => {
          if (globalThis.__FOXMASK__.pgDB) {
            ...
            resolve(globalThis.__FOXMASK__.pgDB);
    ...
    // 创建初始化Promise并保存
    globalThis.__FOXMASK__.pgDBInitPromise = (async () => {
      ...
      try {
        ...
        // 保存实例引用
        globalThis.__FOXMASK__.pgDB = db;
        ...
        // 清空初始化Promise，允许下次重试
        globalThis.__FOXMASK__.pgDBInitPromise = undefined;
        // 释放可能已获取的锁
        releaseLock();
        throw error;
      } finally {
