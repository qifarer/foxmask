...
export const transformToAiModelList = async ({
  ...
  return produce(chatModels, (draft) => {
    ...
    for (const toAddModel of modelConfig.add) {
      // first try to find the model in FOXMASK_DEFAULT_MODEL_LIST to confirm if it is a known model
      let knownModel = FOXMASK_DEFAULT_MODEL_LIST.find(
        (model) => model.id === toAddModel.id && model.providerId === providerId,
      );

      if (!knownModel) {
        knownModel = FOXMASK_DEFAULT_MODEL_LIST.find((model) => model.id === toAddModel.id);
      ...
      if (knownModel) {
        ...
        // if the model is not in FOXMASK_DEFAULT_MODEL_LIST, add it as a new custom model
        draft.push({
          ...toAddModel,
