import { parseToolCalls } from '@foxmask/model-runtime';
import { ChatErrorType } from '@foxmask/types';

import { MESSAGE_CANCEL_FLAT } from '@/const/message';
import { FOXMASK_OBSERVATION_ID, FOXMASK_TRACE_ID } from '@/const/trace';
...
export const fetchSSE = async (url: string, options: RequestInit & FetchSSEOptions = {}) => {
  ...
  if (response) {
    ...
    if (response.ok) {
      ...
      const traceId = response.headers.get(FOXMASK_TRACE_ID);
      const observationId = response.headers.get(FOXMASK_OBSERVATION_ID);
