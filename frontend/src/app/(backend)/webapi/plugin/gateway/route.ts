import { AgentRuntimeError } from '@foxmask/model-runtime';
import { ChatErrorType, ErrorType, TraceNameMap } from '@foxmask/types';
import { PluginRequestPayload } from '@lobehub/chat-plugin-sdk';
import { createGatewayOnEdgeRuntime } from '@lobehub/chat-plugins-gateway';

import { FOXMASK_AUTH_HEADER, OAUTH_AUTHORIZED, enableNextAuth } from '@/const/auth';
import { FOXMASK_TRACE_ID } from '@/const/trace';
...
export const POST = async (req: Request) => {
  ...
  const authorization = req.headers.get(FOXMASK_AUTH_HEADER);
  ...
  if (trace?.id) {
    res.headers.set(FOXMASK_TRACE_ID, trace.id);
  }
