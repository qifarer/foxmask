...
export class DiscoverService {
  ...
  private _getProviderList = async (): Promise<DiscoverProviderItem[]> => {
    ...
    const [{ FOXMASK_DEFAULT_MODEL_LIST }, { DEFAULT_MODEL_PROVIDER_LIST }] = await Promise.all([
      import('@/config/aiModels'),
      import('@/config/modelProviders'),
    ]);
    const result = DEFAULT_MODEL_PROVIDER_LIST.map((item) => {
      const models = uniq(
        FOXMASK_DEFAULT_MODEL_LIST.filter((m) => m.providerId === item.id).map((m) => m.id),
      );
      const provider = {
        ...item,
        identifier: item.id,
  ...
  getProviderDetail = async (params: {
    ...
    const { FOXMASK_DEFAULT_MODEL_LIST } = await import('@/config/aiModels');
    ...
    if (withReadme) {
      ...
      try {
        ...
        const readmeUrl = urlJoin(
          'https://raw.githubusercontent.com/qifarer/foxmask/refs/heads/main/docs/usage/providers',
    ...
    const result = {
      ...provider,
      models: uniqBy(
        FOXMASK_DEFAULT_MODEL_LIST.filter((m) => m.providerId === provider.id),
        (item) => item.id,
      ),
      readme,
  ...
  private _getRawModelList = async (): Promise<DiscoverModelItem[]> => {
    ...
    const { FOXMASK_DEFAULT_MODEL_LIST } = await import('@/config/aiModels');
    const result = FOXMASK_DEFAULT_MODEL_LIST.map((item) => {
      const identifier = (item.id.split('/').at(-1) || item.id).toLowerCase();
      const providers = uniq(
        FOXMASK_DEFAULT_MODEL_LIST.filter(
          (m) =>
  ...
  getModelCategories = async (params: CategoryListQuery = {}): Promise<CategoryItem[]> => {
    ...
    const { q } = params;
    const { FOXMASK_DEFAULT_MODEL_LIST } = await import('@/config/aiModels');
    let list = FOXMASK_DEFAULT_MODEL_LIST;
    if (q) {
      const originalCount = list.length;
      list = list.filter((item) => {
  ...
  getModelDetail = async (params: {
    ...
    const result = {
      ...
      providers: providers.map((item) => ({
        ...item,
        model: FOXMASK_DEFAULT_MODEL_LIST.find((m) => {
