...
import { FOXMASK_LOCALE_COOKIE } from '@/const/locale';
import { FOXMASK_THEME_APPEARANCE } from '@/const/theme';
...
const defaultMiddleware = (request: NextRequest) => {
  ...
  const theme =
    request.cookies.get(FOXMASK_THEME_APPEARANCE)?.value || parseDefaultThemeFromCountry(request);
  ...
  const locale =
    explicitlyLocale ||
    ((request.cookies.get(FOXMASK_LOCALE_COOKIE)?.value || browserLanguage) as Locales);
  ...
  logDefault('User preferences: %O', {
    ...
    hasCookies: {
      locale: !!request.cookies.get(FOXMASK_LOCALE_COOKIE)?.value,
      theme: !!request.cookies.get(FOXMASK_THEME_APPEARANCE)?.value,
    },
    locale,
    theme,
  });
  ...
  // https://github.com/qifarer/foxmask/issues/5876
  ...
  // refs: https://github.com/qifarer/foxmask/pull/5866
...
const nextAuthMiddleware = NextAuthEdge.auth(async (req) => {
  ...
  // refs: https://github.com/qifarer/foxmask/pull/1323
...
const clerkAuthMiddleware = clerkMiddleware(
  ...
  {
    // https://github.com/qifarer/foxmask/pull/3084
