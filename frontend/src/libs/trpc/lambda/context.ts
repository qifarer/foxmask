...
export const createLambdaContext = async (request: NextRequest): Promise<LambdaContext> => {
  ...
  const authorization = request.headers.get(FOXMASK_AUTH_HEADER);
  ...
  log('FoxMask Authorization header: %s', authorization ? 'exists' : 'not found');
  ...
  if (oidcEnv.ENABLE_OIDC) {
    ...
    const oidcAuthToken = request.headers.get(FOXMASK_OIDC_AUTH_HEADER);
  ...
  // If OIDC is not enabled or validation fails, try FoxMask custom Header and other authentication methods
  if (enableClerk) {
