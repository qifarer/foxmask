...
import { ClientSecretPayload, FOXMASK_AUTH_HEADER } from '@/const/auth';
...
export const createAsyncRouteContext = async (request: NextRequest): Promise<AsyncContext> => {
  ...
  const lobeChatAuthorization = request.headers.get(FOXMASK_AUTH_HEADER);

  log('Authorization header present: %s', !!authorization);
  log('FoxMask auth header present: %s', !!lobeChatAuthorization);
  ...
  if (!lobeChatAuthorization) {
    log('No FoxMask authorization header found');
    throw new Error('No FoxMask authorization header found');
  }
  ...
  try {
    ...
    log('Decrypting FoxMask authorization');
